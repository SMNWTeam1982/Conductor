...
impl DriverStation {
    ...
    pub async fn get_mode(&mut self) -> Mode {
        let _ = self.input_tx
            .send(DriverStationCommand::GetMode)
            .await;
        match self.output_rx.lock().await.recv().await {
    ...
    pub async fn enable(&mut self) -> bool {
        let _ = self.input_tx
            .send(DriverStationCommand::Enable)
            .await;
        match self.output_rx.lock().await.recv().await {
    ...
    pub async fn disable(&mut self) -> bool {
        let _ = self.input_tx
            .send(DriverStationCommand::Disable)
            .await;
        match self.output_rx.lock().await.recv().await {
            Some(DriverStationResponse::Enabled(value)) => value,
            _ => self.enabled
    ...
    pub async fn get_enable_status(&mut self) -> bool {
        let _ = self.input_tx
            .send(DriverStationCommand::GetEnabled)
            .await;
        match self.output_rx.lock().await.recv().await {
            Some(DriverStationResponse::Enabled(value)) => value,
            _ => self.enabled
    ...
    pub async fn estop(&mut self) -> bool {
        let _ = self.input_tx
            .send(DriverStationCommand::Estop)
            .await;
        match self.output_rx.lock().await.recv().await {
    ...
    pub async fn restart_code(&mut self) {
        let _ = self.input_tx
            .send(DriverStationCommand::RestartCode)
            .await;
    }
    ...
    pub async fn get_code_alive(&mut self) -> bool {
        let _ = self.input_tx
            .send(DriverStationCommand::GetCodeAlive)
            .await;
        match self.output_rx.lock().await.recv().await {
    ...
    pub async fn get_simulator(&mut self) -> bool {
        let _ = self.input_tx
            .send(DriverStationCommand::GetSimulator)
            .await;
        match self.output_rx.lock().await.recv().await {
